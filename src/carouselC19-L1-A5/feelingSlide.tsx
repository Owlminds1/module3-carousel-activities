...
const FeelingSlide = ({setIsFirstScreen}:myProps) => {
  ...
  const [isAnswerCorrect, setIsAnswerCorrect] = useState<boolean[]>(new Array(slideData.length).fill(false)); // Track if the answer is correct for each slide
  ...
  const handleCheck = (selectedAnswer: string, correctAnswer: string, index: number) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FCFA] p-5 flex justify-center items-center flex-col gap-5">
      <div className="w-[800px]">
        <h1 className="text-center text-4xl font-bold py-4 text-black">How do you feel</h1>
              ...
              <SwiperSlide key={index}>
                <div className="grid grid-cols-12 w-full">
                  <div className="col-span-6 w-full">
                    <Image src={item.image} width={400} height={100} alt="slide image" />
                  </div>
                  <div className="col-span-6 w-full flex justify-center items-center gap-5 flex-col">
                    <div>
                      <h4 className="text-black text-2xl text-center min-h-[100px]">{item.qustion}</h4>
                      <div className="flex w-full justify-around items-center gap-2">
                        ...
                        <button
                          onClick={() => handleCheck("yellow", item.value, index)}
                          className={`${
        ...
        <div className="w-full flex justify-between items-center mt-5">
          <div
            className={`${
              lastSlide > 0 && isAnswerCorrect[lastSlide] ? "border border-black rounded-full p-3 shadow-inner shadow-[#000000b9] bg-yellow-400" : ""
            } hover:scale-90`}
          >
            <FaArrowLeft
              className={`${
